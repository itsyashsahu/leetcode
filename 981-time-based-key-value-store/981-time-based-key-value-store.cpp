...
public:
    ...
    string get(string key, int timestamp) {
        ...
        if (dist == 0 && M[key].first[dist] > timestamp)
            return "";
        if (dist >= M[key].first.size())
            return M[key].second[M[key].first.size() - 1];
        if (M[key].first[dist] == timestamp) {
            return M[key].second[dist];
        }
